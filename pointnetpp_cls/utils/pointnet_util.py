...
sys.path.append(os.path.join(os.path.dirname(BASE_DIR), 'sampling'))
...
def sample_and_group_all(l3_input_shape,xyz, points, use_xyz=True):
    ...
    batch_size = l3_input_shape[0]
    nsample = l3_input_shape[1]
    print('nsample',nsample)
    new_xyz = tf.constant(np.tile(np.array([0,0,0]).reshape((1,1,3)), (batch_size,1,1)),dtype=tf.float32) # (batch_size, 1, 3)
    idx = tf.constant(np.tile(np.array(list(range(nsample))).reshape((1,1,nsample)), (batch_size,1,1)))
    #idx = tf.constant(tf.tile(tf.reshape(np.array(list(range(nsample))),[1,1,nsample]), (batch_size,1,1)).eval())
...
def pointnet_sa_module(l3_input_shape,xyz, points, npoint, radius, nsample, mlp, mlp2, group_all, is_training, bn_decay, scope, bn=True, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        if group_all:
            nsample = l3_input_shape[1]
            new_xyz, new_points, idx, grouped_xyz = sample_and_group_all(l3_input_shape,xyz, points, use_xyz)
        else:

...
def main():
    ...
    with tf.control_dependencies(update_ops):
        last_layer_train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'logits')
        last_layer_train_op = optimizer.minimize(loss_op + reg_loss, global_step=global_step,
    ...
    print('{}-Number of model parameters: {:d}.'.format(datetime.now(), parameter_num))
    ...
    with tf.Session() as sess:
        ...
        if args.load_ckpt is not None:
            print('{}-Loading checkpoint from {}...'.format(datetime.now(), args.load_ckpt))
        ...
        for batch_idx_train in tqdm(range(batch_num), ncols=60):
            if (batch_idx_train % step_val == 0 and (batch_idx_train != 0 or args.load_ckpt is not None)) \
                ...
                tqdm.write('{}-Checkpoint saved to {}!'.format(datetime.now(), filename_ckpt))
                ...
                tqdm.write('{}-[Val  ]-Average:      Loss: {:.4f}  T-1 Acc: {:.4f}  T-1 mAcc: {:.4f}'
            ...
            xforms_np, rotations_np = pf.get_xforms(batch_size_train,
                                                    ...
                                                    order=setting.rotation_order)
            sess.run(reset_metrics_op)
            sess.run([last_layer_train_op, loss_mean_update_op, t_1_acc_update_op, t_1_per_class_acc_update_op],
                     feed_dict={
                         pts_fts: points_batch,
